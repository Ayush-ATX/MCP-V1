"""Server 1 — mahasamvaad-observability (§5)

Tools:
  - query_and_log       (§5.1)
  - get_dashboard_stats (§5.2)

Resource:
  - query_log://recent  (§5.3)

Transport: stdio (dev) or streamable-http (staging), selectable via CLI arg --transport or
...
mcp = MCPServer(
    name="mahasamvaad-observability",
    version="1.0.0",
    description="QA / Eval observability layer for the MahaSamvaad GR Chat API",
)


# ─────────────────────────────────────────────────────────────────────────────
# Tool: query_and_log (§5.1)
# ─────────────────────────────────────────────────────────────────────────────
@mcp.tool(
    name="query_and_log",
    description=(
        "Send a question to the MahaSamvaad /chat endpoint and log every call to query_log. "
        "Returns the full answer plus metadata. Always writes a row even on failure."
    ),
)
async def query_and_log(
    ...
    """§5.1 — Thin wrapper around POST /api/v1/chat; writes to query_log unconditionally."""
    try:
        ...
        logger.info(
            "query_and_log: logged row %d ok=%s message_id=%s",
            logged_row_id,
            result.ok,
            result.message_id,
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# Tool: get_dashboard_stats (§5.2)
# ─────────────────────────────────────────────────────────────────────────────
@mcp.tool(
    name="get_dashboard_stats",
    description=(
        "Return aggregated analytics over query_log: latency percentiles, token cost trend, "
        "query volume by group, error rate, and outlier queries. Pure read — never calls the API."
    ),
)
async def get_dashboard_stats(
    ...
    """§5.2 — Dashboard aggregates over query_log."""
    try:
        ...
        try:
            ...
            if group_by == "day":
                group_col = "substr(called_at, 1, 10)"
            else:
                group_col = group_by
            ...
            total_row = await conn.execute_fetchall(
                "SELECT COUNT(*) AS n FROM query_log WHERE called_at >= ?", (since,)
            )
...
# ─────────────────────────────────────────────────────────────────────────────
# Resource: query_log://recent (§5.3)
# ─────────────────────────────────────────────────────────────────────────────
@mcp.resource(
    "query_log://recent",
    name="recent_query_log",
    description="Last 50 rows from query_log, newest first. Read-only — no live /chat call.",
    mime_type="application/json",
)
async def recent_query_log() -> str:
    """§5.3 — Read-only; never triggers a live /chat call."""
...
@mcp.resource(
    ...
    description=(
        "Parameterised view of query_log. "
        "limit: number of rows to return (1–500, default 50). "
        "intent: optional filter by intent value (e.g. 'retrieval', 'policy_exploration')."
    ),
    mime_type="application/json",
)
async def recent_query_log_filtered(limit: int = 50, intent: str | None = None) -> str:
    """§5.3 — Parameterised resource variant; never triggers a live /chat call."""
...
@mcp.tool(
    ...
    description="Read recent rows from query_log. limit: max rows (capped at 500). intent: optional filter.",
)
async def list_recent_queries(
    limit: int = 50,
    intent: str | None = None,
) -> dict[str, Any]:
    """Parameterised access to query_log with limit (capped 500) and intent filter."""
    try:
        limit = min(max(1, limit), 500)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MahaSamvaad Observability MCP Server")
