...
async def test_lineage_correctness_identifies_exact_match_and_reversal():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("config.SQLITE_DB_PATH", test_db), patch("config.STORE_DB_PATH", test_db):
            ...
            with patch("tools.lineage_correctness.tool.call_chat", AsyncMock(return_value=mock_chat_exact)):
            ...
            with patch("tools.lineage_correctness.tool.call_chat", AsyncMock(return_value=mock_chat_rev)):
            ...
            with patch("tools.lineage_correctness.tool.call_chat", AsyncMock(return_value=mock_chat_wrong)):
...
async def test_bilingual_parity_multi_pair_and_source_classification():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("config.SQLITE_DB_PATH", test_db), patch("config.STORE_DB_PATH", test_db):
            ...
            with patch("tools.bilingual_parity.tool.call_chat", AsyncMock(side_effect=[
                chat_en_1, chat_mr_1,
                chat_en_2, chat_mr_2,
                chat_en_3, chat_mr_3,
            ])), patch("tools.bilingual_parity.tool.default_judge_client.evaluate_json", AsyncMock(side_effect=[
...
async def test_bilingual_parity_handles_agent_timeout_and_judge_failure_without_7_fallback():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("config.SQLITE_DB_PATH", test_db), patch("config.STORE_DB_PATH", test_db):
            ...
            with patch("tools.bilingual_parity.tool.call_chat", AsyncMock(side_effect=[chat_timeout_en, chat_mr])), patch(
                "tools.bilingual_parity.tool.default_judge_client.evaluate_json",
...
async def test_refusal_patterns_and_redteam_tool():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("config.SQLITE_DB_PATH", test_db), patch("config.STORE_DB_PATH", test_db):
            ...
            with patch("tools.refusal_redteam.tool.call_chat", AsyncMock(return_value=mock_texas_chat)):
