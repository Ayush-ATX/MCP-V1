...
class Settings(BaseSettings):
    ...
    FALKORDB_HOST: str = Field(default="20.40.56.211")
    FALKORDB_PORT: int = Field(default=6379)
    FALKORDB_USER: str = Field(default="default")
    FALKORDB_PASSWORD: str = Field(default="falkordb")
    FALKORDB_GRAPH_NAME: str = Field(default="mh_gr_kg_dev_v2")
    FALKORDB_SSL: bool = Field(default=False)
    FALKORDB_TIMEOUT_S: float = Field(default=10.0)
    ...
    @field_validator("FALKORDB_HOST", mode="before")
    @classmethod
    def clean_falkordb_host(cls, v: Any) -> str:
        if isinstance(v, str):
            h = v.strip().replace("http://", "").replace("https://", "").strip("/")
            return h
        return str(v)
...
FALKORDB_HOST: str = settings.FALKORDB_HOST
FALKORDB_PORT: int = settings.FALKORDB_PORT
FALKORDB_USER: str = settings.FALKORDB_USER
FALKORDB_PASSWORD: str = settings.FALKORDB_PASSWORD
FALKORDB_GRAPH_NAME: str = settings.FALKORDB_GRAPH_NAME
FALKORDB_SSL: bool = settings.FALKORDB_SSL
FALKORDB_TIMEOUT_S: float = settings.FALKORDB_TIMEOUT_S
