...
Strategy: patch `mahasamvaad_eval.clients.chat_api_client.httpx` with a fake module whose
...
from mahasamvaad_eval.clients.chat_api_client import call_chat
...
async def test_successful_call():
    ...
    with patch("mahasamvaad_eval.clients.chat_api_client.httpx.AsyncClient", MockClient):
...
async def test_timeout_returns_ok_false():
    ...
    MockClient = _make_mock_client_class(handler)
    with patch("mahasamvaad_eval.clients.chat_api_client.httpx.AsyncClient", MockClient), \
         patch("mahasamvaad_eval.clients.chat_api_client._RETRY_BACKOFF_S", 0.0):
