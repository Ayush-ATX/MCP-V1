...
from mahasamvaad_eval.tools.reformulation import (
...
async def test_generate_paraphrases_valid_json():
    ...
    with patch("mahasamvaad_eval.tools.reformulation._call_reformulation_model", new=AsyncMock(return_value=MOCK_VARIANTS_JSON)):
...
async def test_generate_paraphrases_strips_code_fence():
    ...
    with patch("mahasamvaad_eval.tools.reformulation._call_reformulation_model", new=AsyncMock(return_value=MOCK_VARIANTS_WITH_FENCE)):
        result = await generate_paraphrases("What is the MRTP Act?", n=3)
...
async def test_generate_paraphrases_invalid_json():
    ...
    with patch("mahasamvaad_eval.tools.reformulation._call_reformulation_model", new=AsyncMock(return_value="Sorry, I cannot help.")):
...
async def test_generate_paraphrases_non_array_json():
    ...
    with patch("mahasamvaad_eval.tools.reformulation._call_reformulation_model", new=AsyncMock(return_value='{"text": "...", "kind": "formal"}')):
...
async def test_generate_paraphrases_n_clamped():
    ...
    with patch("mahasamvaad_eval.tools.reformulation._call_reformulation_model", new=mock_model):
...
async def test_generate_paraphrases_missing_api_key():
    ...
    with patch("mahasamvaad_eval.tools.reformulation.config") as mock_cfg:
        mock_cfg.REFORMULATION_API_KEY = ""
        mock_cfg.LLM_JUDGE_API_KEY = ""
...
def test_streaming_model_uses_supplied_configuration_and_collects_content():
    ...
    with patch("mahasamvaad_eval.tools.reformulation.config") as mock_cfg, patch(
        "mahasamvaad_eval.tools.reformulation.OpenAI"
    ) as openai:
...
async def test_generate_paraphrases_model_failure_returns_existing_error_shape():
    with patch(
        "mahasamvaad_eval.tools.reformulation._call_reformulation_model",
...
async def test_stability_all_agree():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        from mahasamvaad_eval.storage import db as real_store
        with patch("mahasamvaad_eval.tools.reformulation._log_chat_call", new=_make_mock_log_chat(gr_seq)), \
             patch("mahasamvaad_eval.tools.reformulation.get_db", new=lambda: real_store.get_db(db_path)), \
             patch("mahasamvaad_eval.tools.reformulation.execute_with_retry", new=real_store.execute_with_retry):
            # Init schema
...
async def test_stability_keeps_at_most_three_concurrent_chat_calls():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        db_path = os.path.join(tmpdir, "test.db")
        from mahasamvaad_eval.storage import db as real_store
        with patch("mahasamvaad_eval.tools.reformulation._log_chat_call", new=mock_log), \
             patch("mahasamvaad_eval.tools.reformulation.get_db", new=lambda: real_store.get_db(db_path)), \
             patch("mahasamvaad_eval.tools.reformulation.execute_with_retry", new=real_store.execute_with_retry):
            conn = await real_store.get_db(db_path)
            await conn.close()
...
def test_dotenv_loading_and_overrides():
    """Settings loads values from .env while preserving existing env vars."""
    from mahasamvaad_eval.config import Settings
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch.dict(os.environ, {"REFORMULATION_API_KEY": "original_override_val"}, clear=False):
            test_settings = Settings(_env_file=env_file)
            assert test_settings.REFORMULATION_API_KEY == "original_override_val"
            assert test_settings.REFORMULATION_MODEL == "nvidia/nemotron-3.5-lightning-30b-a3b"
            assert test_settings.REFORMULATION_BASE_URL == "https://integrate.api.nvidia.com/v1"
